...
import { Send, FileSignature, MessageSquarePlus } from 'lucide-react';
...
export function NotesClient({ tripId, initialNotes, initialMessages }: NotesClientProps) {
  ...
  const handleAddNote = async () => {
    ...
    try {
      await createNoteAction(tripId, title, content);
    } catch (err) {
    ...
    <div className="flex flex-col gap-0">
      ...
      <div className={cn(
        ...
        <div className="flex-1 p-5 overflow-y-auto bg-surface-elevated/40 grid gap-4 content-start">
          {messages.map((msg) => {
            const isOwn = msg.sender.name === 'Алина';
            return (
              <div
                key={msg.id}
                className={cn(
